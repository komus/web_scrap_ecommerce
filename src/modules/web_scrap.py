...
def get_page_content(url:str, samples_num: int, category: str) -> List:
    ...
    if read_site.status_code == 200:
        ...
        for info in soup.find_all(class_="sg-col-4-of-12 s-result-item s-asin sg-col-4-of-16 sg-col sg-col-4-of-20"):
            ratings = info.find(class_="a-icon-alt").text
            review_count = info.find("span", "a-size-base").text
            ...
            data.append(
                {
                    ...
                    'price': price,
                    'coupon': info.find("span", "a-size-base s-highlighted-text-padding aok-inline-block s-coupon-highlight-color"),
                })
